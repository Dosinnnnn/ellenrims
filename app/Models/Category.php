...
class Category extends Model
{
    ...
    public function child_cat(){
        return $this->hasMany('App\Models\Category','parent_id','id')->where('status','aktif');
    }
    public static function getAllParentWithChild(){
        return Category::with('child_cat')->where('is_parent',1)->where('status','aktif')->orderBy('title','ASC')->get();
    }
    public function products(){
        return $this->hasMany('App\Models\Product','cat_id','id')->where('status','aktif');
    }
    public function sub_products(){
        return $this->hasMany('App\Models\Product','child_cat_id','id')->where('status','aktif');
    }
    ...
    public static function countActiveCategory(){
        $data=Category::where('status','aktif')->count();
        if($data){
            return $data;
        }
        return 0;
